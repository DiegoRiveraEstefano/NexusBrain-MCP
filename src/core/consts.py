...
RECORD_DECISION_SUCCESS_TEMPLATE = (
    "✅ **Decision successfully saved** to project memory (ID: `{record_id}`)."
)
...
BLAST_RADIUS_NO_AFFECTED_NODES_TEMPLATE = (
    "Node `{node_id}` exists, but has no components depending on it."
)
BLAST_RADIUS_RESULT_ITEM_TEMPLATE = "{index}. **File:** `{file_path}` (ID: `{node_id}`)\n"
BLAST_RADIUS_FOOTER = "\n *Consider reviewing these files if making structural changes.*"

# get_execution_flow tool
EXEC_FLOW_NO_DEPENDENCIES_TEMPLATE = (
    "Node `{node_id}` has no registered outgoing dependencies (it is a leaf/pure function)."
)
...
EXEC_FLOW_RESULT_ITEM_TEMPLATE = "- Calls: `{file_path}` (ID: `{dep_id}`)\n"
...
SEMANTIC_SEARCH_RESULT_ITEM_TEMPLATE = (
    "**Result {index}** (File: `{file_path}`) | Similarity: {score:.3f}\n"
)
SEMANTIC_SEARCH_CODE_BLOCK_TEMPLATE = "```\n{content}\n```\n\n"

# ingest_repo tool
INGEST_REPO_RESULT_TEMPLATE = (
    "Repo ingested. status={status} files={files} chunks={chunks} edges={edges} time={time:.2f}s"
)

# Ingestion Script UI Messages
INGEST_ERROR_PATH_NOT_FOUND_TEMPLATE = (
    "[red]Error: Path '{path}' does not exist or is not a directory.[/red]"
)
...
INGEST_STEP_2_INSERTING_TEMPLATE = (
    "[yellow]2/3 Generating Embeddings and inserting {count} nodes..."
)
